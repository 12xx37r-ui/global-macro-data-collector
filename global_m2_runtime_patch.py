...
# Runtime-safe network policy.
# Global M2 is monthly, so a failed endpoint must never stall the whole collector.
TIMEOUT = (4, 10)
OPTIONAL_TIMEOUT = (3, 7)
...
def _fetch_us_fred(session: requests.Session) -> dict[str, Any]:
    r = _get_retry(session, FRED_US_M2, attempts=2, timeout=TIMEOUT)
    rows = []
...
def _fetch_ecb(session: requests.Session) -> dict[str, Any]:
    r = _get_retry(session, ECB_M2_CSV, headers={"Accept": "text/csv"}, attempts=2, timeout=TIMEOUT)
...
def _fetch_boj(session: requests.Session) -> dict[str, Any]:
    r = _get_retry(session, BOJ_M2_PAGE, attempts=1, timeout=OPTIONAL_TIMEOUT)
    soup = BeautifulSoup(r.text, "html.parser")
    ...
    for url in links[:2]:
        try:
            rr = _get_retry(session, url, attempts=1, timeout=OPTIONAL_TIMEOUT)
...
def _fetch_pbc(session: requests.Session) -> dict[str, Any]:
    ...
    r = _get_retry(session, PBC_SEARCH, headers=headers, attempts=1, timeout=OPTIONAL_TIMEOUT)
    soup = BeautifulSoup(r.text, "html.parser")
    candidates: list[str] = []
    ...
    for url in candidates[:3]:
        try:
            rr = _get_retry(session, url, headers=headers, attempts=1, timeout=OPTIONAL_TIMEOUT)
...
def build_global_m2(session: requests.Session | None = None) -> dict[str, Any]:
    ...
    if weight_sum < 0.50 or len(usable) < 2:
        return {
            ...
            "methodology": "Requires at least two usable regions and 50% strategic coverage; otherwise the composite abstains instead of fabricating a global value. Network calls use bounded retries/timeouts so optional regional sources cannot stall the workflow.",
            "is_proxy": False,
        }
