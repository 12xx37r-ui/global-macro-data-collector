...
class TestCards(unittest.TestCase):
    ...
    def test_card11_failed_future_gate_is_explicitly_reference_only(self):
        q={'passed':True}
        qf={'passed':False}
        ...
        c9={'market_signal':'good','current':44,'forecasts':{'3m':{'forecast':48,'quality_gate':qf}},'data_quality':{'core_completeness':100}}
        c10={'market_signal':'bad','current':46,'forecasts':{'3m':{'forecast':48,'quality_gate':q}},'data_quality':{'core_completeness':100}}
        c12={'market_signal':'good','predictive_validation':{'groups':{}},'data_quality':{'completeness':100}}
        out=build_card11(c8,c9,c10,c12)
        self.assertFalse(out['future_quality_gate']['passed'])
        self.assertEqual(out['future_validation_status'],'REFERENCE_ONLY')
        self.assertIn('검증 제한',out['future_regime'])
        self.assertIn('(검증 제한)',out['asset_environment']['global_equities'])
        self.assertTrue(out['current_quality_gate']['passed'])
        self.assertEqual(out['current_quality_gate']['scope'],'current_state_data_quality')
        self.assertFalse(out['quality_gate']['passed'])
