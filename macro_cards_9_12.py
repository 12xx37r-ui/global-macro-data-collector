...
def _global_registry(global_m2:dict[str,Any], card8:dict[str,Any], card9:dict[str,Any], card10:dict[str,Any], card11:dict[str,Any], card12:dict[str,Any], composite_oos:dict[str,Any])->dict[str,Any]:
    ...
    aggregate_conf=float(((global_m2.get('forecast_confidence') or {}).get('score')) or (sum(confs)/len(confs) if confs else 35))
    sub_val=global_m2.get('validated_subcomposite_forecast_validation_3m') or {}
    entries.append(finalize({'id':'global_m2_3m','label':'Global M2 YoY 3m','horizon':'3m','current':global_m2.get('current'),'forecast':global_m2.get('forecast'),'status':'VALIDATED' if full_m2 else 'REFERENCE_ONLY','usable':bool(full_m2),'benchmark_safe':True,'component_gates':comp_pass,'composite_validation':comp_val,'validated_subcomposite_validation':sub_val,'reason':'all regional gates and final composite OOS passed' if full_m2 else ('70%+ fixed-weight regional subset has OOS edge; full four-region validation awaits longer China history' if (sub_val.get('quality_gate') or {}).get('passed') else 'benchmark-safe, but regional/final-composite validation is incomplete')}, samples=int(comp_val.get('samples') or sub_val.get('samples') or 0), confidence=aggregate_conf))
    ...
    liq_recon=liq.get('reconstructed_oos_validation_3m') or {}; recon_pass=bool((liq_recon.get('quality_gate') or {}).get('passed'))
    liq_samples=max([int((x or {}).get('samples') or 0) for x in liq_oos.values()] or [0])
    if not liq_samples: liq_samples=int(liq_recon.get('samples') or 0)
    liq_conf=min(85.0,55.0+max(0.0,float(liq_recon.get('skill_pct') or 0.0))*.6+max(0.0,float(liq_recon.get('direction_accuracy') or 50.0)-50.0)*.35) if recon_pass else 45.0
    entries.append(finalize({'id':'forward_liquidity_3m','label':'Forward liquidity composite 3m','horizon':'3m','current':liq.get('score'),'forecast':None,'status':'VALIDATED' if liq_pass else 'RECONSTRUCTED_VALIDATED' if recon_pass else 'REFERENCE_ONLY','usable':liq_pass,'input_validation':liq.get('validation_status'),'oos':liq_oos,'reconstructed_oos':liq_recon,'reason':'real published-snapshot OOS gate passed' if liq_pass else 'past-only reconstructed OOS passed; awaiting real published-snapshot promotion' if recon_pass else 'awaiting/failed reconstructed and real published-snapshot OOS validation'},samples=liq_samples,confidence=liq_conf))
    ...
    return {'schema_version':'1.1','generated_at_utc':generated,'engine':'Global macro engine','entries':entries,'summary':{'validated':sum(1 for x in entries if x.get('status')=='VALIDATED'),'reconstructed_validated':sum(1 for x in entries if x.get('status')=='RECONSTRUCTED_VALIDATED'),'reference_only':sum(1 for x in entries if x.get('status')=='REFERENCE_ONLY'),'degraded':sum(1 for x in entries if x.get('degradation_status')=='DEGRADED')},'policy':'Only horizon-specific forecasts that pass their own gate are promoted; rolling OOS deterioration automatically downgrades eligible forecast blocks. Composite signals may show reconstructed validation evidence, but production usability still requires realized published-snapshot OOS validation.'}
